...
import advancetext from '../images/advance.gif';
import newYear2020 from '../images/newYear.gif';
...
var c, ctx,
  ...
  D = 1e-3, newYearDate = new Date('1/1/2020 00:00'); // strength of drag (air resistance);
// Tree Variable
var spirals, thetamin = 0,
  thetamax = 6 * Math.PI,
  period = 5,
  linespacing = 1 / 30,
...
export default class BackgroundFirework extends Component {
  state = { showAdvance: true, showNewYear: false, today: new Date('12/25/2019 00:00') }
  getInitialState() {
    return { showAdvance: true, showNewYear: false }
  }
  init() {
    c = document.getElementById('backgroundFireworks')
    ctx = c.getContext('2d')
    ctx.font = '50px Comic Sans MS, cursive, TSCu_Comic, sans-serif'
    ctx.lineWidth = 5; ctx.lineJoin = 'round'; ctx.globalAlpha = 2 / 3
    ctx.strokeStyle = ctx.fillStyle = '#1f2f90'
    c.width = ((window.innerWidth <= 1024) ? window.innerWidth : 1024)
  ...
  handleResponsive(width) {
    ...
    if (width <= 380) {
      yscreenoffset = c.height + (c.height / 14);
      xscreenoffset = c.width / 2
      ycamera = 2.5
      zcamera = -2
    }
    else if (width <= 600) {
      yscreenoffset = c.height
      xscreenoffset = c.width / 2
      ycamera = 2.5
      zcamera = -2
    }
      ...
      yscreenoffset = c.height - (c.height / 12);
      xscreenoffset = c.width / 2
      ycamera = 2.5
      zcamera = -2
    }
  ...
  advanceWishes = () => {
    ...
    if (ToDate >= newYearDate.getTime()) {
      this.setState({ showAdvance: false, showNewYear: true })
      // console.log(this.state.showAdvance, this.state.showNewYear);
      return false;
    }
    if (ToDate >= new Date('12/26/2019 00:00').getTime()) {
      this.setState({ showAdvance: true, showNewYear: true })
      // console.log(this.state.showAdvance, this.state.showNewYear);
      return false;
    }
  ...
  render() {
    return (
      <div>
        ...
        <div>
          {this.state.showAdvance ? <img className="advanceText" width='220' height='400' src={advancetext} alt="Merry Christmas" /> : null}
          {this.state.showNewYear ? <img className="newYear2020" width='220' height='400' src={newYear2020} alt="Happy New Year" /> : null}
          <div className="newTextname"><TextAnimate /> </div>
        </div>
